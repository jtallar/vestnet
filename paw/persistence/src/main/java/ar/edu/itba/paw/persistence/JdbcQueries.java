...
public class JdbcQueries {
    ...
    static final String PROJECT_FIND_BY_OWNER =
            ...
            "SELECT " +
                    ...
                    "JOIN " + PROJECT_CATEGORIES_TABLE + " pcat ON (p.id = pcat.project_id) " +
                    "JOIN " + CATEGORIES_TABLE + " cat ON (pcat.category_id = cat.id) " +
                    "LEFT OUTER JOIN (SELECT project_id, COUNT(*) AS amount FROM messages WHERE accepted IS NULL GROUP BY project_id) mess ON (mess.project_id = p.id) " +
                    "WHERE p.owner_id = ? " +
                    "ORDER BY p.id ";
